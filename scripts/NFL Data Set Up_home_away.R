...
agg_off <- pbp %>%
  ...
  group_by(season, home_team, game_id) %>%
...
### Down %s ####
agg_downs <- pbp %>%
  group_by(season, home_team, game_id , down) %>%
...
team_agg_base <-  left_join(agg_off, pen_yds) %>%
  left_join(agg_downs) %>%
  dplyr::left_join(agg_def , join_by(season, game_id , home_team))

# AVG FEATURES ####
prev_avg <- function(x) (cumsum(x) - x )/(row_number() - 1 )
curr_avg <- function(x) (cumsum(x)/(row_number()))

team_agg_home <- team_agg_base %>%
  left_join(
    team_agg_base %>% group_by(season , home_team) %>%
...
agg_off <- pbp %>%
  ...
  group_by(season, away_team, game_id) %>%
  summarise(passyds = sum(passyds , na.rm = TRUE),
            pass.comp = sum(complete_pass , na.rm = TRUE),
            pass.att = sum(pass_attempt, na.rm = TRUE) ,
            pass_plays = sum(pass , na.rm = T) ,
...
### Down %s ####
agg_downs <- pbp %>%
  group_by(season, away_team, game_id , down) %>%
  summarize_at(c("first_1" , "first_2" , "first_3" ,"first_4") , mean, na.rm = T) %>%
  group_by(season, away_team, game_id) %>%
  summarize_at(c("first_1" , "first_2" , "first_3" ,"first_4") , sum, na.rm = T)
### Penalty Yds ####
...
team_agg_base <-  left_join(agg_off, pen_yds) %>%
  left_join(agg_downs) %>%
  dplyr::left_join(agg_def , join_by(season, game_id , away_team))

team_agg_away <- team_agg_base %>%
  left_join(
    team_agg_base %>% group_by(season , away_team) %>%
      mutate_at(vars(-group_cols() & -game_id), prev_avg) %>%
      rename_at(vars(-group_cols() & -game_id), function(x)paste0(x, ".prev_avg")) ## Avgs up to now ####
  ) %>%
  left_join(
...
alldata <- games %>%
  left_join(team_agg_home , by = join_by(game_id  , season , home_team) , suffix = c("" , ".y")) %>% select(-ends_with(".y")) %>%
  left_join(team_agg_away , by = join_by(game_id  , season , away_team) , suffix = c("" , ".away")) # %>%
