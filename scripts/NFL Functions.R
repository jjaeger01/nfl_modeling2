...
make_predictions <- function(test_data ,
  ...
  if(standard_predict == T){
    pred_base <- test_data %>%
      mutate(pred = predict(fit_ , test_data))
  } else{
    if(outcome_type == "cont"){
      if(outcome_ == "home_away_score"){
        ...
        pred_base <- test_data %>%
          mutate(pred = predict.train(fit_ , test_data))
      }
    }
    if(outcome_type == "cat"){
      pred_base <- test_data %>%
  ...
  if(sparse_df == T){
    predicted_data <- predicted_data %>%  select(game_id , pred , method , outcome , outcome_type , pred_time)
    # if(outcome_type == "cat"){
...
eval_pred <- function(predicted_data ,
  ...
  test_outcomes <- game_outcomes(predicted_data) %>%
    left_join(predicted_data %>%
                select(game_id , pred , method), by = join_by(game_id)  , suffix = c("" , ".close")
    ) %>%
  ...
  if(outcome_ == "spread_line_neg"){
      test_outcomes <- test_outcomes %>% mutate(
        pred_home_score = NA ,
        pred_away_score = NA ,
        pred_linedif = (pred*-1) + spread_line ,
        pred_cover = ifelse(pred_linedif > 0 , 1 , 0) ,
        pred_cat_ = pred_cover ,
        outcome_ = home_cover ,
        pred_win = ifelse(pred > 0 , 1 , 0) ,
